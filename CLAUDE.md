...
2. **`internal/pool.Pool`** owns a single `wazero.Runtime`, a single `CompiledModule` (compiled once at startup — slow, 10–60s), and a `worker.Pool` of N Python worker processes (default 4, `--workers N`). Each request instantiates a fresh WASM module from the shared compiled artifact and acquires one worker for its full duration.
...
6. **`internal/worker.Worker`** owns one host-side `python3 shim/worker.py <sockPath>` subprocess, which pre-imports numpy/scipy/pandas and serves length-prefixed JSON over a Unix socket. `Worker.Call`/`Probe`/`Ping`/`Shutdown` hold a per-worker mutex; **`internal/worker.Pool`** wraps N of them so concurrent `/run` requests run in parallel against separate workers. Pool members spawn at server start, are health-pinged on each acquire, and are restarted on failure.
...
- **ndarray transport is asymmetric.** WASM cannot read or write `/dev/shm`, so two encodings coexist on `worker.Arg`:
  - **Worker ↔ Go** uses **binary via shm files** — `shim/worker.py` writes raw `arr.tobytes()` to `/dev/shm/chr-worker-*` and returns the path in `Value`. Go reads it (`encodeNDArrayResult`) and unlinks. Conversely `resolveArg` writes inbound bytes into a fresh shm block and forwards the path to the worker.
  - **WASM ↔ Go** uses **inline base64** in the `B64` field (`worker.Arg.B64`, JSON `b64`). On WASM-out, `_ser_array` in `bootstrap.py` emits `{type: "ndarray", b64, shape, dtype}`. On WASM-in, the dispatcher fills `B64` so `_deser_tensor` can rebuild a `_TensorView`. There is no Go-side shm copy for results — re-passing an ndarray always re-encodes inline anyway, so the copy was dead weight.
- **Probe responses are cached** server-side in `Dispatcher.probeCache` (sync.Map). Probes are deterministic for the worker's lifetime, so `numpy.X.__probe__` only round-trips to a worker once across all requests. ~28% latency reduction on warm requests.
...
- **Per-request allocation, shared compilation.** The `wazero.CompiledModule` is reused across requests; only `InstantiateModule` happens per request. Do not re-compile per request — startup will balloon.
- **Concurrency is bounded by the worker pool.** Each `/run` acquires one worker for its full duration; up to `--workers` requests can run in parallel. Per-worker socket I/O is serial (the worker is single-threaded internally). Across workers it's truly parallel.
- **`shmMgr` is per-bridge-call, not per-request.** Each `dispatch()` invocation creates and tears down its own `ShmManager`. A bridge-heavy request (thousands of calls) used to leak shm pages until end of /run, blowing past Docker's default 64MB `/dev/shm` under concurrency. Don't widen the scope.
- **wazero needs `WithCloseOnContextDone(true)`.** Set in `internal/pool.New`. Without it, ctx cancellation does not actually close in-flight WASM modules at host-call boundaries, so `timeout_sec` is a no-op for bridge-looped user code.
- **Bridge goroutine cleanup needs both pipe ends closed.** `dispatcher.Run` closes `pipeStdoutW` *and* `pipeStdinR` when wazero returns. The latter is non-obvious: if wazero killed the WASM module mid-cycle, the bridge goroutine may be blocked in `pipeStdinW.Write` waiting for a WASM reader that no longer exists. Closing the read end unsticks it.
